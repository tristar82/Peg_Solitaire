...
class Peg:
        ...
        while True:
            try:
                with open(input("Enter solution filename in full: "), 'r') \
                    ...
                    n_moves_found = len(file_contents_clean)
    ...
    def verify_input_char(self, input_char, peg_dict, location):
        ...
        peg_dict: str
            Name of dictionary variable containing the character
            to position coords lookup

        location: str
            String to indentify if the position entered is the
            origin or destination. Used for more descriptive error messages
        ...
        while input_char not in peg_dict.keys():
            input_char = input(
                "Please enter a single character position for "
                + "{} as found on the board: ".format(location.upper()))
    ...
    def auto_export_to_file(self):
        ...
        to prevent accidental file overwriting.
        '''
        while True: # dont need any of this as unique
            try:
                now_dt = datetime.datetime.now()
                date_time = now_dt.strftime("%Y_%m_%d__%H_%M_%S")
...
class Board:
    ...
    def print_board(self, pegs_on_board):
        '''
        Function that prints the current state of the board (i.e. pegs
        and holes). Also prints the numner of pegs remaining on the board
        '''
        board = self.board
...
class Menu:
    ...
    def initiate_game(self):
        print("Options are (R)ules, Load (S)olution File,"
              + " (P)lay game or (E)xit")
